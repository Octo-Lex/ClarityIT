...
class GenerateG3Tests(unittest.TestCase):
    ...
    def test_column_digest_gate_couples_generation_expression(self):
        """Production-coupled regression: the generated adoption SQL must
        contain BOTH the ``generation_expression`` information_schema column
        in the digest query AND the ``genexpr`` format field.  Removing
        either from the generator would cause this test to fail."""
        adoption_sql = self.files_a[g3.ADOPTION_SQL].decode()
        self.assertIn(
            "generation_expression",
            adoption_sql,
            "adoption SQL column digest does not project generation_expression "
            "from information_schema — the drift gate cannot detect "
            "expression-only changes on generated columns",
        )
        self.assertIn(
            "genexpr:",
            adoption_sql,
            "adoption SQL column digest format string lacks the genexpr field "
            "— generation_expression is not incorporated into the digest",
        )
