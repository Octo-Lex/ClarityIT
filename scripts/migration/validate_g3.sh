...
# Complete catalog projection snapshot for drift/atomicity comparisons.
# Captures roles, memberships, schemas, relations, columns, grants, ownership,
# and row counts — a complete enough projection to prove zero writes.
catalog_snapshot() {
    local name="$1"
    docker exec "$name" psql -U clarityit -d clarityit -tAc "
        SELECT
          (SELECT count(*) FROM pg_roles WHERE rolname !~ '^pg_') || ':' ||
          (SELECT count(*) FROM pg_auth_members m JOIN pg_roles r ON r.oid=m.member JOIN pg_roles r2 ON r2.oid=m.roleid WHERE r.rolname !~ '^pg_' AND r2.rolname !~ '^pg_') || ':' ||
          (SELECT count(*) FROM pg_namespace WHERE nspname NOT IN ('pg_catalog','information_schema','pg_toast') AND nspname NOT LIKE 'pg_temp%') || ':' ||
          (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname NOT IN ('pg_catalog','information_schema','pg_toast') AND c.relkind IN ('r','S','i','v')) || ':' ||
          (SELECT count(*) FROM information_schema.columns WHERE table_schema NOT IN ('pg_catalog','information_schema')) || ':' ||
          (SELECT count(DISTINCT n.nspname||'.'||c.relname||'|'||a.privilege_type||'|'||pg_get_userbyid(a.grantee))
           FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace, aclexplode(c.relacl) a
           WHERE n.nspname NOT IN ('pg_catalog','information_schema','pg_toast') AND c.relkind IN ('r','S','i','v')) || ':' ||
          (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname NOT IN ('pg_catalog','information_schema','pg_toast')) || ':' ||
          (SELECT count(*) FROM (
            SELECT table_schema||'.'||table_name FROM information_schema.tables WHERE table_schema NOT IN ('pg_catalog','information_schema')
            UNION ALL
            SELECT 'permissions.count'||count(*)::text FROM public.permissions
          ) x)"
}
...
# Build a deliberately-failing adoption variant: the real SQL up to COMMIT,
# but with an injected exception just before the final demotion.
sed '/^ALTER ROLE clarityit LOGIN INHERIT/i\
