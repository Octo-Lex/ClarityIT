...
-- ============================================================
DO $g6_p2_adopt_preflight$
BEGIN
    IF current_database() <> 'clarityit' THEN
        RAISE EXCEPTION 'G6 P2 adoption requires database clarityit, got %', current_database();
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = current_user AND rolsuper) THEN
        RAISE EXCEPTION 'G6 P2 adoption requires a PostgreSQL superuser';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pgcrypto') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires extension pgcrypto';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'citext') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires extension citext';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'clarityit' AND rolsuper) THEN
        RAISE EXCEPTION 'G6 P2 adoption requires the P3 bootstrap superuser clarityit';
    END IF;
    ...
    IF (SELECT encode(public.digest(convert_to(string_agg(
        ...
        RAISE EXCEPTION 'G6 P2 adoption source column properties drifted (digest mismatch)';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname IN (
        ...
        'legacy_ext_owner','g6_p2_adopt_admin')) THEN
        RAISE EXCEPTION 'G6 P2 adoption is single-shot: a target/legacy identity already exists';
    END IF;
    IF EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'platform') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires no existing platform schema';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='action_outcomes' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.action_outcomes';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_effect_results' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_effect_results';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_evaluation_runs' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_evaluation_runs';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_evaluation_scenario_results' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_evaluation_scenario_results';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_identities' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_identities';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_intentions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_intentions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_runs' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_runs';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='agent_tool_grants' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.agent_tool_grants';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='alerts' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.alerts';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='approval_decisions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.approval_decisions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='approval_policies' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.approval_policies';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='approval_requests' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.approval_requests';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='artifact_document_versions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.artifact_document_versions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='artifact_documents' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.artifact_documents';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='artifact_meeting_data' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.artifact_meeting_data';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='artifact_templates' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.artifact_templates';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='artifacts' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.artifacts';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='asset_actions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.asset_actions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='assets' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.assets';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='audit_logs' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.audit_logs';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='bootstrap_lock' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.bootstrap_lock';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='context_bundles' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.context_bundles';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='context_edge_evidence' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.context_edge_evidence';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='context_edges' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.context_edges';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='context_nodes' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.context_nodes';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='context_relation_reviews' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.context_relation_reviews';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='docs' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.docs';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='idempotency_keys' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.idempotency_keys';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='incidents' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.incidents';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='integration_api_keys' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.integration_api_keys';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='invitations' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.invitations';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='knowledge_chunks' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.knowledge_chunks';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='knowledge_collection_items' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.knowledge_collection_items';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='knowledge_collections' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.knowledge_collections';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='knowledge_items' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.knowledge_items';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='mfa_challenges' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.mfa_challenges';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='mfa_recovery_codes' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.mfa_recovery_codes';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='object_comments' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.object_comments';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='object_links' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.object_links';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='object_storage_refs' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.object_storage_refs';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='objects' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.objects';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='outbox_events' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.outbox_events';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='password_reset_tokens' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.password_reset_tokens';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='permissions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.permissions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='platform_roles' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.platform_roles';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='proxmox_mutation_windows' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.proxmox_mutation_windows';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='recommendation_evidence' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.recommendation_evidence';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='refresh_tokens' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.refresh_tokens';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='remediation_proposals' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.remediation_proposals';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='remediation_steps' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.remediation_steps';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='role_permissions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.role_permissions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='roles' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.roles';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='saved_knowledge_answers' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.saved_knowledge_answers';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='storage_objects' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.storage_objects';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='team_access_grants' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.team_access_grants';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='team_memberships' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.team_memberships';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='teams' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.teams';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='tool_registry' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.tool_registry';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='user_mfa_factors' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.user_mfa_factors';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='user_platform_roles' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.user_platform_roles';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='user_sessions' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.user_sessions';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='user_webauthn_credentials' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.user_webauthn_credentials';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='users' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.users';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relname='work_items' AND c.relkind='r') THEN
        RAISE EXCEPTION 'G6 P2 adoption requires product table public.work_items';
    END IF;
END
$g6_p2_adopt_preflight$;
...
-- ============================================================
CREATE ROLE g6_p2_adopt_admin NOLOGIN SUPERUSER;
SET SESSION AUTHORIZATION g6_p2_adopt_admin;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'work.items.edit.own') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'work.items.update.own') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, '53c0f4d2-6fec-5d57-84ca-ed58a8dfc19d'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'work.items.edit.own' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'work.items.edit.own');
            DELETE FROM public.permissions WHERE name = 'work.items.edit.own';
        ELSE
            UPDATE public.permissions SET name = 'work.items.update.own',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'work.items.edit.own';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'work.items.update.own') THEN
            RAISE EXCEPTION 'G6 D016: neither work.items.edit.own nor work.items.update.own exists';
        END IF;
    END IF;
    -- work.items.edit.any -> work.items.update.any
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'work.items.edit.any') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'work.items.update.any') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, '4f2499c2-ad5d-5215-94cf-1919ca9fa865'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'work.items.edit.any' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'work.items.edit.any');
            DELETE FROM public.permissions WHERE name = 'work.items.edit.any';
        ELSE
            UPDATE public.permissions SET name = 'work.items.update.any',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'work.items.edit.any';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'work.items.update.any') THEN
            RAISE EXCEPTION 'G6 D016: neither work.items.edit.any nor work.items.update.any exists';
        END IF;
    END IF;
    -- projects.edit -> projects.update
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'projects.edit') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'projects.update') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, '6a53d14f-8ca0-5be7-9a77-f8775d36efaa'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'projects.edit' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'projects.edit');
            DELETE FROM public.permissions WHERE name = 'projects.edit';
        ELSE
            UPDATE public.permissions SET name = 'projects.update',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'projects.edit';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'projects.update') THEN
            RAISE EXCEPTION 'G6 D016: neither projects.edit nor projects.update exists';
        END IF;
    END IF;
    -- incidents.edit.own -> incidents.update.own
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'incidents.edit.own') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'incidents.update.own') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, '678fd8d6-56e9-5335-8b72-06ec2cb09c97'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'incidents.edit.own' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'incidents.edit.own');
            DELETE FROM public.permissions WHERE name = 'incidents.edit.own';
        ELSE
            UPDATE public.permissions SET name = 'incidents.update.own',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'incidents.edit.own';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'incidents.update.own') THEN
            RAISE EXCEPTION 'G6 D016: neither incidents.edit.own nor incidents.update.own exists';
        END IF;
    END IF;
    -- incidents.edit.any -> incidents.update.any
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'incidents.edit.any') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'incidents.update.any') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, '4c73278f-fd39-585c-a8a8-2508e016bde3'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'incidents.edit.any' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'incidents.edit.any');
            DELETE FROM public.permissions WHERE name = 'incidents.edit.any';
        ELSE
            UPDATE public.permissions SET name = 'incidents.update.any',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'incidents.edit.any';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'incidents.update.any') THEN
            RAISE EXCEPTION 'G6 D016: neither incidents.edit.any nor incidents.update.any exists';
        END IF;
    END IF;
    -- docs.edit.own -> docs.update.own
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'docs.edit.own') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'docs.update.own') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, 'bdb6f96a-8577-5763-9a48-19adff491206'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'docs.edit.own' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'docs.edit.own');
            DELETE FROM public.permissions WHERE name = 'docs.edit.own';
        ELSE
            UPDATE public.permissions SET name = 'docs.update.own',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'docs.edit.own';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'docs.update.own') THEN
            RAISE EXCEPTION 'G6 D016: neither docs.edit.own nor docs.update.own exists';
        END IF;
    END IF;
    -- docs.edit.any -> docs.update.any
    IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'docs.edit.any') THEN
        IF EXISTS (SELECT 1 FROM public.permissions WHERE name = 'docs.update.any') THEN
            INSERT INTO public.role_permissions (role_id, permission_id)
            SELECT role_id, '341bd87b-d622-525d-8c06-94308da39f99'::uuid FROM public.role_permissions rp
            JOIN public.permissions p ON p.id = rp.permission_id
            WHERE p.name = 'docs.edit.any' ON CONFLICT DO NOTHING;
            DELETE FROM public.role_permissions WHERE permission_id =
                (SELECT id FROM public.permissions WHERE name = 'docs.edit.any');
            DELETE FROM public.permissions WHERE name = 'docs.edit.any';
        ELSE
            UPDATE public.permissions SET name = 'docs.update.any',
                action = replace(action, 'edit', 'update'),
                resource = replace(resource, 'edit', 'update')
            WHERE name = 'docs.edit.any';
        END IF;
    ELSE
        IF NOT EXISTS (SELECT 1 FROM public.permissions WHERE name = 'docs.update.any') THEN
            RAISE EXCEPTION 'G6 D016: neither docs.edit.any nor docs.update.any exists';
        END IF;
    END IF;
...
INSERT INTO platform.source_profiles (profile_id, schema_fingerprint, postgres_version, postgres_major, extensions, roles_digest, source_commit, approved_by, approved_at) VALUES (
    '7b5b8b87-3467-5fd5-9bac-3dbcdd858178', '57c2b64597f8df459043681a4faaf3c789e0eb17883d3ea9585dffac654121cb', 'PostgreSQL 16', 16, '["pgcrypto","citext","pg_trgm"]'::jsonb, '2273a104fa6145ebe699ffc570da41941d49df4584ee2b093f323ce8d5a0a7c3', 'b9d15877583e84c45ab5478dfaa6087966926fc5', 'G6-TERMINAL-CLOSURE-AUTH-2026-08-11', '2026-08-11T00:00:00Z');
INSERT INTO platform.schema_revisions (version, name, checksum, source_commit, applied_at, applied_by, execution_ms, success)
VALUES ('0001', 'adopt-p2-v32', '1021adefe8b5edaae13010a713cdde594f084a66b9d4012940603ee4a94e0508', current_setting('g3.source_commit', true), '2026-08-03T00:00:00Z', 'g6-p2-adoption-artifact', 0, true);
RESET ROLE;
