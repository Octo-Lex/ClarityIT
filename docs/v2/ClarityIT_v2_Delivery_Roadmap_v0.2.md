...
9. The CT 150 KES/IAM placement is a development-only bootstrap profile. Its accepted single-host risk does not establish production conformance and cannot be promoted in place.
...
- establish the CT 150 development trust bootstrap with KES-backed object encryption, dedicated least-privilege writer and independent read-only verifier identities, object lock/versioning, retention/legal hold, durable audit capture, exact-version read-back, and denied retention-bypass tests;
...
WP-00 exits only under its existing G6 acceptance boundary. For development, G6 may accept CT 150 as one shared failure domain after the required in-host trust and custody controls pass and the durability limitation is recorded in the Environment Trust and Evidence Custody Deployment Profile. That decision authorizes project development only; it is not production-readiness evidence. WP-00 evidence becomes the input to WP-01, and pilot or pattern implementation cannot substitute for missing baseline, restore, CI, or development-profile evidence.
...
- a production trust topology that places IAM, the KES/root-of-trust tier, evidence storage, application administration, and the audit receiver in approved independent failure domains; uses KMS/HSM-protected keys, separate writer and read-only verifier identities, immutable cross-domain replication, and tested key/data recovery; and provisions production fresh rather than promoting CT 150 in place;
...
- production IAM and KES survive an approved failure-domain loss; every evidence object is encrypted, retained, independently replicated, retrieved by exact version through the verifier identity, and hash-verified; retention bypass, legal-hold removal, and explicit version deletion are denied to non-break-glass identities;
- no CT 150 IAM realm, KES root material, bucket credential, service identity, or evidence key is reused in production, and the production deployment manifest proves fresh trust establishment;
...
|---|---|---|---|
| WP-00 G6 | Baseline and CI are reliable for development | Existing WP-00 evidence pack plus CT 150 development-profile control evidence and explicit single-host risk acceptance | Existing WP-00 authorities; Security and Operations accept the development exception |
...
| RG-10 | Production service is operable and safe | Executable deployment contract, clause truth, drift, rollback/forward recovery, SLO, DR, security, accessibility, migration, support evidence, independent trust topology, fresh production identities/keys, and immutable cross-domain custody proof | Product, engineering, operations, security, database, quality |
...
| Production trust | Production IAM, KES/root-of-trust, evidence storage, application administration, and audit custody sharing the CT 150 development failure domain | Must remain zero |
| Promotion | CT 150 IAM realms, service identities, root keys, bucket credentials, or evidence keys reused in production | Must remain zero |
...
3. `docs/v2/ClarityIT_v2_Environment_Trust_and_Evidence_Custody_Deployment_Profile_v0.1.md`

### Existing artifacts to update

| Artifact | Required change |
|---|---|
| `docs/v2/README.md` | Add all three documents to the authority index; state that only WP-00 is currently formal and later package names are proposed until approved. |
...
| Product Definition section 6 | State that the layered architecture is logical; adopt the bounded CT 150 development placement and separate production trust profile. |
| Kernel section 16 and Appendix B | Add phase-specific trust placement, no-in-place promotion, and the Environment Trust and Evidence Custody Deployment Profile. |
| Migration Specification sections 15-17 | Add the development exception, production gate, conformance scenario, and required profile artifact. |
| WP-00 plan | Add a boundary note that no later pattern implementation is part of WP-00, that its G6 remains the prerequisite, and that CT 150 acceptance is development-only. |
...
4. Deploy and verify KES and dedicated IAM identities on CT 150 against the development profile; record the single-host risk without describing it as production-compliant custody.
5. Create WP-01 only after WP-00 evidence is accepted; use RG-01 as its exit gate.
6. Create the Generic Compute Adapter and Proxmox profile documents during WP-02 planning.
7. Create the First-Release Experience Specification before WP-03 implementation completion.
8. Derive the Context Overlay and Executor Credential Broker contracts during WP-01 planning; complete their live conformance in WP-02 and WP-03.
9. Create the Runtime Capability Profile and Migration Contract before WP-04 route implementation; create the signed Skill Package and Signal/Routine contracts before WP-05 and WP-06 implementation.
10. Approve the production trust topology and exercise the Executable Deployment Contract before WP-10 release-candidate approval; production is provisioned fresh and must not inherit CT 150 trust material.
11. Do not schedule WP-04 through WP-10 against calendar dates until R1 evidence and team capacity are known.
